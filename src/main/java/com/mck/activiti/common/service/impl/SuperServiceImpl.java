package com.mck.activiti.common.service.impl;
...
import com.mck.activiti.common.config.exception.IdempotencyException;
import com.mck.activiti.common.config.exception.LockException;
import com.mck.activiti.common.lock.DistributedLock;
import com.mck.activiti.common.lock.ZLock;
...
public class SuperServiceImpl<M extends BaseMapper<T>, T> extends ServiceImpl<M, T> implements ISuperService<T> {
    ...
    @Override
    public boolean updateIdempotency(T entity, DistributedLock locker, String lockKey, Wrapper<T> countWrapper, String msg) throws Exception {
        if (locker == null) {
            throw new LockException("DistributedLock is null");
        }
        if (StrUtil.isEmpty(lockKey)) {
    ...
    @Override
    public boolean updateIdempotency(Wrapper<T> updateWrapper, DistributedLock locker, String lockKey, Wrapper<T> countWrapper, String msg) throws Exception {
        if (locker == null) {
            throw new LockException("DistributedLock is null");
        }
        if (StrUtil.isEmpty(lockKey)) {
